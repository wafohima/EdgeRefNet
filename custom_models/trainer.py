...
class CDTrainer():
    ...
    def _forward_pass(self, batch_data):
        ...
        if 'A' not in self.batch or 'B' not in self.batch:
            self.logger.write("Warning: Batch missing 'A' or 'B' images. Skipping forward pass.\n")
            self.G_pred = None
            self.G_final_pred = None
            return

        img_in1, img_in2 = self.batch['A'].to(self.device), self.batch['B'].to(self.device)
        
        try:
            model_output = self.net_G(img_in1, img_in2) 
            self.G_pred = model_output
        except Exception as e:
            self.logger.write(f"Error during model forward pass: {e}\n")
            dummy_output_shape = (img_in1.shape[0], self.n_class, img_in1.shape[2], img_in1.shape[3])
            self.G_pred = [torch.zeros(dummy_output_shape, device=self.device), 
                           torch.zeros(dummy_output_shape, device=self.device)] 
            self.G_final_pred = self.G_pred[-1] 
            return

        if isinstance(self.G_pred, (list, tuple)) and len(self.G_pred) > 0:
            self.G_final_pred = self.G_pred[-1]
        elif isinstance(self.G_pred, torch.Tensor):
            self.G_final_pred = self.G_pred
            if not (isinstance(self.G_pred, (list, tuple)) and len(self.G_pred) >=2): 
                 self.G_pred = [self.G_final_pred, self.G_final_pred] 
        else:
            self.logger.write(f"Warning: G_pred from net_G is not in expected format. Type: {type(self.G_pred)}. Creating dummy G_final_pred.\n")
            dummy_output_shape = (img_in1.shape[0], self.n_class, img_in1.shape[2], img_in1.shape[3])
            self.G_final_pred = torch.zeros(dummy_output_shape, device=self.device)
            self.G_pred = [self.G_final_pred, self.G_final_pred]
