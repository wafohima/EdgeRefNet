...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if args.net_G == 'EdgeRefNet':
        ...
        feature_channels = 64
        net = STANet(in_c=feature_channels, ds=1, mode='PAM') # 'PAM' is the more advanced version from the paper
...
class ResNet(torch.nn.Module):
    ...
    def forward_single(self, x):
        ...
        x = self.SA1(x)
        edge_feature.append(x)  
        x = self.resnet.maxpool(x)

        x_4 = self.resnet.layer1(x)  
        x_4 = self.SA2(x_4)
        edge_feature.append(x_4)  

        x_8 = self.resnet.layer2(x_4)  
        x_8 = self.SA3(x_8)
        edge_feature.append(x_8)  
